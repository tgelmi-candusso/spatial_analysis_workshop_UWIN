...
car.int <- st_intersection(coyotes_buff, res_roads) #has the
...
Estimate tree density, mean height, and total crown cover.
...
writeVector(tree.int, "maps/trees_camera_buffers.shp")

##start from here
tree.int<-st_read("maps/trees_camera_buffers.shp")
tree.int <- sf::st_transform(tree.int, crs="EPSG:5070")
...
Estimate mean vegetation density (NDVI) and total building density

```{r}
ex <- extract(NDVI, coyotes_buff_t, fun='mean', na.rm=TRUE)
